...
export default function CustomerChatPage() {
  ...
  async function handleAcceptUpsell(offerId: string) {
    await db
      .from("upsell_offers")
    ...
    setUpsellOffers((prev) => ({
      ...prev,
      [offerId]: { ...prev[offerId], status: "accepted" },
    }));
  }

  async function handleDeclineUpsell(offerId: string) {
    await db
      .from("upsell_offers")
    ...
    setUpsellOffers((prev) => ({
      ...prev,
      [offerId]: { ...prev[offerId], status: "declined" },
    }));
  }
  ...
  return (
    <div className="flex h-[calc(100dvh-4rem)] flex-col">
      ...
      <div className="flex items-center gap-3 border-b px-4 py-3">
        ...
        <div>
          <h1 className="text-base font-semibold">Chat con lavador</h1>
          ...
          if (msg.message_type === "upsell_offer" && msg.metadata?.upsell_id) {
            ...
            return (
              <UpsellCard
                ...
                offer={offer}
                isCustomer={true}
                onAccept={() => handleAcceptUpsell(offer.id)}
                onDecline={() => handleDeclineUpsell(offer.id)}
              />
            );
          }
